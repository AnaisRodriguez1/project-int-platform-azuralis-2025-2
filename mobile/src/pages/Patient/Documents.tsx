...
export function DocumentsPatient({ hideHeader = false }: { hideHeader?: boolean }) {
  ...
  // Agrupa Comité Oncológico (todos los que tengan "comite"+"oncologico") y aplica filtro por tipo
  const filterDocuments = () => {
    ...
    const comiteDocs = base.filter((d) => {
      const t = normalize(d.title);
      return t.includes("comite") && t.includes("oncologico");
    });
    const otherDocs = base.filter((d) => {
      const t = normalize(d.title);
      return !(t.includes("comite") && t.includes("oncologico"));
    });
